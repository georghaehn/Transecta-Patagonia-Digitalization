...
## 2.2 Check species not in the Argentinian checklist based on The Plant List
...
### 2.2.2 Synonyms
Identify the species names being synonyms of accepted names. These names will be compared with the Argentinian checklist to check whether the accepted names are already included.
```{r, warning = FALSE}
tosynonyms <- checked2 |> 
...
Create list of unresolved or unmatched species. These species will be checked manually and eventually added to the Argentinian checklist to allow import into Turboveg 2.
```{r}
unres <- checked2 |> 
...
In total we have `r nrow(unres)` unresolved species. `r unres.tag |> filter(LEVEL == "SPECIES") |> pull(LEVEL) |> length()` are expected to be correct at the species level. `r unres.tag |> filter(LEVEL == "GENUS") |> pull(LEVEL) |> length()` are correct at the genus level. `r unres.tag |> filter(LEVEL == "UNKNOWN") |> pull(LEVEL) |> length()` species remain unknown. `r unres.tag |> filter(TAG == "Lichen") |> pull(LEVEL) |> length()` were tagged as lichens. `r unres.tag |> filter(TAG == "Moss") |> pull(LEVEL) |> length()` were tagged as moss. `r unres.tag |> filter(TAG == "Fungi") |> pull(LEVEL) |> length()` were tagged as fungi. `r unres.tag |> filter(TAG == "Algae") |> pull(LEVEL) |> length()` were tagged as algae. `r unres.tag |> filter(TAG == "Fern") |> pull(LEVEL) |> length()` were tagged as fern.  
...
#There are 8 duplicated entries. They are also duplicated in the original source.
#We leave it to the future user to decide how to deal with those cases and leave them inside and the column duplicate. From our perspective we recommend to remove them.

df.out <- df.out |> 
  mutate(Duplicate = ifelse(PlotID %in%
                             (df.out[duplicated(df.out),] |> 
                              pull(PlotID)), TRUE, FALSE))
...
```{r, warning = FALSE}
openxlsx::write.xlsx(df.out, file = file.path(output_path, "DT-Transecta-Patagonia.xlsx"))
openxlsx::write.xlsx(checklist, file = file.path(output_path, "Species-Transecta-Patagonia.xlsx"))
```
...
header0 <- header0 |> 
 ...
 #coalesce the fields environment and Note
  ...
  #fill up with F for those rows where at least one column on formation is assigned
...
saveWorkbook(out, file = file.path(output_path, "WB-Transecta-Patagonia.xlsx"), overwrite = TRUE)

save(DT1, header1, file = file.path(output_path, "R-Image-Transecta-Patagonia.RData"))
write_csv(header1, file.path(output_path, "Header-Transecta-Patagonia.csv"))
write_csv(DT1, file.path(output_path, "DT-Transecta-Patagonia.csv"))
```
